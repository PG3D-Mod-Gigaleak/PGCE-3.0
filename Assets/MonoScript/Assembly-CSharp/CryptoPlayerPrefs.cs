...
public static class CryptoPlayerPrefs
{
	...
	{
		Rijndael rijndaelForKey = getRijndaelForKey(Password);
	...
	private static string DecryptString(string cipherText, string Password)
	{
		...
		{
			rijndaelDict = new Dictionary<string, Rijndael>();
		}
		byte[] cipherData = Convert.FromBase64String(cipherText);
		Rijndael rijndaelForKey = getRijndaelForKey(Password);
		byte[] bytes = DecryptString(cipherData, rijndaelForKey);
