...
//                         Debug.LogError("DamagePlayer");
	...
	public void DamageTarget()
	{
		if (timer <= 0f) 
		{
            ...
            {
				target.transform.parent.GetComponent<InitializeHealthbar>().DamageNPC("Zombie",coopDamage,multiplayerDamage,damageCooldown,_weaponManager,originalObject,damageSender);
		    }
            if (target.tag == "BodyCollider")
		    {
				target.transform.parent.GetComponent<InitializeHealthbar>().DamageNPC("Player",coopDamage,multiplayerDamage,damageCooldown,_weaponManager,originalObject,damageSender);
		    }
		}
		timer = damageCooldown;
	}
