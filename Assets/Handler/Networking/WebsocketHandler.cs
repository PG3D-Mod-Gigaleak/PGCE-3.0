...
namespace handler.networking
{
	public class WebsocketHandler : MonoBehaviour
	{
		...
		private static long SeshID = -1;
		private static long SeshKey = -1;
		...
		private static IEnumerator RealInit()
		{
			...
			CallAction("ensure_ws_alive", (string data) => {
				...
				// .. dont bother with decrypting the result dict here
				Dictionary<string, object> resultDictionary = JsonConvert.DeserializeObject<Dictionary<string, object>>(data);
				EditorOnlyDebug.Log($"response: {resultDictionary["response"]}");
				if ((string)resultDictionary["response"] == "success")
				{
					EditorOnlyDebug.Log($"setting session key");
					SeshKey = Convert.ToInt64((string)resultDictionary["d_ky"]);
					EditorOnlyDebug.Log($"setting session id");
					SeshID = Convert.ToInt64((string)resultDictionary["s_id"]);
					EditorOnlyDebug.Log($"setting ws is alive");
					WSIsAlive = true;
				}
		...
		public static void CallAction(string action, ActionAnswered callback, Dictionary<string, object> args)
		{
			...
			string prefix = SeshID == -1 ? "" : (SeshID + "__ID-SPLITTER|^|ID-SPLITTER__");
			CurrentConnection.Send(prefix + sargs);
		}
		#endregion
		...
		public static long ReturnFunnyLittleThing(float value, float istart, float istop, float ostart, float ostop)
		{
			return (long)(ostart + (value - istart) * (ostop - ostart) / (istop - istart));
		}
